setwd("E:\\Desktop_Laptop_Xfer\\Nano-Enabled_Antimicrobials_Working_Group\\R21\\Preliminary_Data_R21")


# The parsing ----
pacman::p_load(tidyverse, rvest)

dirs <- c("./output_ancest/output/", # ancestral
          "./output_lms3/output/", # No silver 20 passages
          "./output_lms6/output/") # Silver 20 passages

file_list <- paste0(dirs, "index.html")
...
for(filename in file_list){
  ...
  condensed_name <- str_split_i(filename, pattern = "/", 2)
  master_list[[condensed_name]] <- mutation_list
}

## Bind the variants ---- 
master_list %>%
  keep(names(.) %in% names(master_list)) %>%
  map2(names(.), ~ .x$`Predicted mutations` %>% 
         mutate(ID_strain = gsub("output_", "", .y))) %>% 
  bind_rows() %>% 
...
parse_row <- function(row) {
  ...
  if (str_detect(row$mutation, "→")) { # finds the SNPS
    ...
    if (str_detect(gene_tmp, "/")) {
      gene_v <- c(
        str_split_1(gene_tmp, "/") %>% str_trim() %>% str_replace_all(" ", ""),
        str_split_1(gene_tmp, "/") %>% str_trim() %>% str_replace_all(" ", "")
      )
    } else {
...
parsed_rows$treatment<- c(rep("ancestral", 3),
                            rep("broth only", 4),
                            rep("AgNP-spherical", 4))
parsed_rows$strain <- c(rep(0, 3),
                        rep(1, 4),
                        rep(2, 4))
